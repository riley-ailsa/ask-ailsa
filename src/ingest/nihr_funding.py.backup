...
class NihrFundingScraper:
    ...
    def _parse_sections_from_nav(self, base_url: str, soup: BeautifulSoup) -> List[Dict[str, Any]]:
        """
        DEPRECATED: Old navigation-based section parser.
        Replaced by _parse_sections_from_headings for better coverage.
        """
        # Call new heading-based parser
        sections = self._parse_sections_from_headings(soup, base_url)
    ...
    def _extract_resources(
        ...
        section_objs = self._parse_sections_from_headings(soup, base_url)
